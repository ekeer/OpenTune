...
|---------|---------|---------|--------------|
| `PianoRollComponent` + `PianoRollRenderer` | live tree | 单层场景宿主 + 主层渲染路径 | 这是当前 v2.1 已收敛后的唯一正确 PianoRoll 绘制结构，Phase 22 不能绕开它另建 overlay/第二层。[VERIFIED: Source/Standalone/UI/PianoRollComponent.cpp][VERIFIED: .planning/PROJECT.md] |
...
- `.planning/PROJECT.md` - v2.1 目标、out-of-scope、主张与约束
