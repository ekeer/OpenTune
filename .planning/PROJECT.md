...
- 2026-05-18 Piano Roll empty-space seek intent: `mouseDown` on empty space arms `EmptySpacePending`, `mouseUp` within 12 logical-pixel threshold seeks to click time, drag beyond threshold converts to active tool drag. One mouse gesture = one intent. `setTool()` cancels pending state. 2026-05-19 correction: LineAnchor main-edit clicks bypass empty-space seek and stay LineAnchor tool input; timeline-ruler seek remains immediate. Plan: `.planning/plans/2026-05-18-piano-roll-empty-space-seek-intent.md`.
...
- ✓ `NotesPrimary` 与 `CorrectedF0Primary` 两套交互语义已完成第一轮迁移：notes-first 当前会在 hand-draw / line-anchor 后自动选中受影响 notes；corrected-f0-first 允许 line-anchor segment selection，但参数面板不再把 LineAnchor segment 元数据作为 retune target，LineAnchor 输出真相固定为 committed `f0Data` — current live tree
...
当前两套 fixed scheme 已完成 owner 正确的产品收口：`NotesPrimary` 在手绘 / line-anchor 成功提交后会自动选中受影响 notes，并且只允许 voiced frame 被编辑；`CorrectedF0Primary` 允许 line-anchor segment selection 用于段选择交互，但不再把 RetuneSpeed 路由到 LineAnchor segment 元数据，LineAnchor 与 HandDraw 一样以 committed `f0Data` 作为唯一输出真相。`showUnvoicedFrames` / `noteNameMode` / `showChunkBoundaries` 也已经进入 `AppPreferences` shared state，通过 shared preferences page、View menu、Standalone/VST3 editor 同步到 PianoRoll 渲染。当前 undo/redo 也已经完成 processor-owned content/placement-aware affected-range result chain 收口；自动化验证已重新跑通，剩余验证缺口只在手工旅程确认。
