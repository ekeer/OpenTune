...
- Issue: WAS — `OpenTuneDocumentController` and `OpenTunePlaybackRenderer` held raw `OpenTuneAudioProcessor* processor_` pointers, routing ARA content birth and playback through processor-side ARA birth/playback helpers. Now REMOVED.
- Resolution: DC now receives stores via `connectToStores(shared_ptr<MaterializationStore>, shared_ptr<SourceStore>, shared_ptr<ResamplingManager>, shared_ptr<F0InferenceService>, scheduleAsyncWork, reclaimCallback)`. DC directly creates materializations via `birthMaterializationForModification` (read ARA audio → upsample → sourceStore->createSource → materializationStore->createMaterialization → notifyContentChanged). Renderer reads `MaterializationStore*` directly → `getPlaybackReadSource()` → free function `readPlaybackAudio()` in `PlaybackAudioReader.h`. F0 extraction async is scheduled via `scheduleAsyncWork_` callback (Processor's F0ExtractionService worker pool), not via detached `std::thread` in DC.
- Remaining concern: `readPlaybackAudio` still exists as both a free function (`PlaybackAudioReader.h`) and a member function on `OpenTuneAudioProcessor` — should converge to one implementation.
