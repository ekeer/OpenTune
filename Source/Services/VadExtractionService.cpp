#include "VadExtractionService.h"
...
VadExtractionService::VadExtractionService(int workerCount, size_t maxQueueSize)
    : queue_(maxQueueSize)
...
VadExtractionService::~VadExtractionService()
{
...
uint64_t VadExtractionService::makeRequestKey(uint64_t materializationId, int trackId, int placementIndex)
{
    if (materializationId != 0) return materializationId;
    return (static_cast<uint64_t>(static_cast<uint32_t>(trackId)) << 32)
         | static_cast<uint32_t>(placementIndex);
}

VadExtractionService::SubmitResult VadExtractionService::submit(uint64_t requestKey,
                                                                  ExecuteFn execute,
                                                                  CommitFn commit)
{
...
bool VadExtractionService::isActive(uint64_t requestKey) const
{
    std::lock_guard<std::mutex> lock(entriesMutex_);
    return activeEntries_.find(requestKey) != activeEntries_.end();
}

void VadExtractionService::cancel(uint64_t requestKey)
{
    std::lock_guard<std::mutex> lock(entriesMutex_);
    activeEntries_.erase(requestKey);
}

void VadExtractionService::workerLoop()
{
    while (running_.load(std::memory_order_acquire)) {
        ...
        uint64_t expectedToken = 0;
        {
            ...
            if (it == activeEntries_.end()
                || it->second->token.load(std::memory_order_relaxed) != task.token) {
                continue;
            }
        ...
        try {
            ...
            AppLogger::error("[VadExtractionService] Unknown exception during task execution");
            result.success = false;
            result.errorMessage = "execute_exception";
        }

        if (result.requestKey == 0) result.requestKey = task.requestKey;
        ...
        bool shouldCommit = false;
        {
            ...
            auto it = activeEntries_.find(task.requestKey);
            if (it != activeEntries_.end()
                && it->second->token.load(std::memory_order_relaxed) == expectedToken) {
                shouldCommit = true;
        ...
        juce::MessageManager::callAsync(
            [commit = std::move(task.commit), result = std::move(result)]() mutable {
